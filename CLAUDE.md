...
- **Binders UI v1** ✅ — `/collection/binders` lists every binder + a virtual "Toutes mes cartes" entry that links to the unfiltered Collection. Tap a binder → Collection deeplinked with `?binderId=...`. Each row has a kebab → bottom sheet with Renommer / Supprimer; delete orphans items (sets `binderId` to null) inside a Dexie transaction. `/collection/binders/new` and `/collection/binders/:id/edit` share `BinderFormPage` (name + description + 8-icon radiogroup). Long-press menu on a Collection item now exposes a **Déplacer dans un classeur** action via `MoveToBinderSheet`. Drag-to-reorder, "Vider", custom binder header on the deeplinked view, and backend sync of binders are deferred follow-ups. Entry point: Profile → Mes classeurs.
...
- **Collection extras** — virtualization at 100+ items, pull-to-refresh. (Sort + view-mode prefs persisted in Dexie ✅; filter is URL-driven for deeplinks ✅; long-press menu ✅: `useLongPress` on `CollectionItemCard` + `MoreVertical` kebab fallback for keyboard a11y → opens `ItemActionSheet` with Modifier / Dupliquer / Supprimer. "Déplacer" depends on the binders UI and is deferred. Stack view doesn't expose the menu since rows are aggregated.)
