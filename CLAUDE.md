...
- **OAuth buttons on Login/Signup** ✅ — Google + Apple buttons on both pages call `useSignInWithOAuth` → `backend.signInWithOAuth(provider)`. Supabase backend triggers the redirect; the dev mock fakes an immediate sign-in (synthetic email `<provider>-user@oauth-mock.local`) so the flow is testable offline. Misconfigured providers (disabled in Supabase, missing keys) map to `AuthError('oauth_provider_not_configured')` and surface as a translated toast. Activation steps for Google + Apple live in `docs/deploy.md`.
